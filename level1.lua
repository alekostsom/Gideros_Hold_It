...
function level1:init()
	...
	self.custom7 = self:customObject(0, 800, "water")
	...
	--Create a "back to menu" button with a click event listener
	local backBtn = Button.new(Bitmap.new(Texture.new("./ArtAssets/Images/GUI/backBtnUP.png")), 
									Bitmap.new(Texture.new("./ArtAssets/Images/GUI/backBtnDOWN.png")))
	backBtn:setPosition(10, 10) 
	self:addChild(backBtn)
	
	backBtn:addEventListener("click",
		function()
			sceneManager:changeScene("menu", 1, SceneManager.fade, easing.linear)
		end)
	...
	function level1:onTouchesBegin(event)
		for i=1, 4 do
	...
	function level1:onTouchesMove(event)
	...
	function level1:onTouchesEnd(event)
		if mouseJoint~=nil then
...
function level1:wall(x, y, width, height)
...
function level1:customObject(x, y, name)
    --create customObj bitmap object from ball graphic
    local customObj = Bitmap.new(Texture.new("./ArtAssets/Images/" .. name .. ".png"))
...
function level1:touchObject(name)
	--create touchObj bitmap object from the appropriate image
	local touchObj = Bitmap.new(Texture.new("./ArtAssets/Images/" .. name .. ".png"))
...
function level1:onBeginContact(e)
	...
	--local sound = Sound.new("./ArtAssets/Sound Effects/blip.WAV")
	--local sound2 = Sound.new("./ArtAssets/Sound Effects/click_x.WAV")
